		...
		if (profiles) {
			...
			return {
				...
				businessHours: {
					...
					businessConfig: businessHoursConfig?.map(({ attrs }) => ({
						dayOfWeek: attrs.day_of_week,
						mode: attrs.mode,
						openTime: attrs.open_time ? parseInt(attrs.open_time) : undefined,
						closeTime: attrs.close_time ? parseInt(attrs.close_time) : undefined
					}))
				}
			}
		}
